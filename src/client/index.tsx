...
function SshBody(props) {
  ...
  async function connect() {
    setBusy(true); setErr('')
    try { await callApi('connect', { host: hostV, user: userV, port: Number(portV) || 22, password: passV || undefined }); await refresh() }
    catch (e) { setErr(String((e && e.message) || e)) } finally { setBusy(false) }
  }
  async function run() {
    setBusy(true); setErr('')
    try {
      const r = await callApi('exec', { command: cmdV, waitMs: 15000 })
  ...
  async function disconnect() {
    setBusy(true); setErr('')
    try { await callApi('disconnect'); await refresh() }
    catch (e) { setErr(String((e && e.message) || e)) } finally { setBusy(false) }
  }
  ...
  const onKey = (e) => { if (e.key === 'Enter' && connected && !busy) run() }
  ...
  return createElement('div', { className: 'dshssh-body' },
    createElement('div', { className: 'dshssh-head' },
      ...
      createElement('button', { className: 'dshssh-btn dshssh-clear', onClick: clearHistory }, '清空记录'),
    ),
    createElement('div', { className: 'dshssh-form' },
      ...
      createElement('button', { className: 'dshssh-btn', disabled: busy, onClick: connect }, '连接'),
    ),
    createElement('div', { className: 'dshssh-form' },
      ...
      createElement('button', { className: 'dshssh-btn', disabled: busy || !connected, onClick: run }, '执行'),
      createElement('button', { className: 'dshssh-btn', disabled: busy || !connected, onClick: disconnect }, '断开'),
    ),
