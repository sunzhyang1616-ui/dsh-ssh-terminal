window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SSH_CSS = `
.dshssh-body{position:relative;display:flex;flex-direction:column;gap:8px;height:100%;box-sizing:border-box;padding:10px;color:var(--dsw-alias-label-primary,inherit);}
...
.dshssh-context-menu{position:absolute;z-index:1000;display:flex;flex-direction:column;min-width:140px;padding:4px;border:1px solid var(--dsw-alias-border-l2,rgba(148,163,184,.4));border-radius:7px;background:var(--dsw-alias-bg-layer-3,#252529);box-shadow:0 8px 24px rgba(0,0,0,.35);}
...
`;
		...
		function SshBody(props) {
			...
			return (0, react.createElement)("div", {
				className: "dshssh-body",
				onClick: () => setContextMenu(null)
			}, (0, react.createElement)("div", { className: "dshssh-head" }, (0, react.createElement)("span", { className: "dshssh-title" }, "SSH 远程终端"), (0, react.createElement)("span", { className: "dshssh-status " + (connected ? "ok" : "off") }, status), (0, react.createElement)("button", {
				...
				onContextMenu: (e) => {
					e.preventDefault();
					e.stopPropagation();
					const body = e.currentTarget.closest(".dshssh-body");
					const rect = body ? body.getBoundingClientRect() : {
						left: 0,
						top: 0,
						width: window.innerWidth,
						height: window.innerHeight
					};
					const x = Math.max(4, Math.min(e.clientX - rect.left, rect.width - 160));
					const y = Math.max(4, Math.min(e.clientY - rect.top, rect.height - 80));
					setContextMenu({
			...
			}, (0, react.createElement)("span", { className: "dshssh-session-dot" + (s.connected ? " ok" : "") }), (0, react.createElement)("span", { className: "dshssh-session-copy" }, (0, react.createElement)("span", { className: "dshssh-session-name" }, s.pinned ? (0, react.createElement)("span", {
				className: "dshssh-pin",
				title: "已置顶"
			}, "★") : null, s.label || s.target || "SSH 连接"), (0, react.createElement)("span", { className: "dshssh-session-meta" }, s.connected ? s.busy ? "执行中" : "已连接" : s.historical ? "历史记录" : "已断开")))) : (0, react.createElement)("div", { className: "dshssh-empty" }, "暂无连接"))), (0, react.createElement)("div", { className: "dshssh-main" }, (0, react.createElement)("div", { className: "dshssh-form" }, (0, react.createElement)("input", {
			...
			}, fmt(s.ts) + "  " + s.text)) : (0, react.createElement)("div", { className: "dshssh-step dshssh-system" }, "还没有操作。")), err ? (0, react.createElement)("div", { className: "dshssh-err" }, String(err)) : null), contextMenu && contextSession ? (0, react.createElement)("div", {
				...
				type: "button",
				onClick: () => togglePinned(contextSession.connectionId)
			}, contextSession.pinned ? "取消置顶" : "置顶"), (0, react.createElement)("button", {
				type: "button",
				className: "danger",
				onClick: () => deleteRecord(contextSession.connectionId)
			}, "删除该连接记录")) : null));
		}
